<template>
  <b-overlay :show="orderLoading" rounded="sm" class="w-100">
    ...
    public fetchOrder(){
      ...
      this.$nextTick(function () {
        ...
        if(!productsStore.productsLoaded){
          ...
          reqs.push(productsStore.loadProduct() as Promise<any>)

        }

        if(this.$route.params.id && !orderStore.orderLoaded) {

          this.orderLoading = true
          reqs.push(orderStore.fetchOrder(this.$route.params.id) as Promise<any>)

          Promise.all(reqs).then( () => {

            orderStore.loadOrderProducts().then(() => {
